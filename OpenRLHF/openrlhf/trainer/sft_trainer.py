...
class SFTTrainer(ABC):
    ...
    def __init__(
        ...
        # MLflow setting
        self._mlflow = None
        if self.strategy.args.use_mlflow and self.strategy.is_rank_0():
            import mlflow
            # Save the mlflow module so it can be accessed later.
            self._mlflow = mlflow
            mlflow.set_tracking_uri(self.strategy.args.use_mlflow)
            if hasattr(self.strategy.args, "mlflow_experiment_name"):
                mlflow.set_experiment(self.strategy.args.mlflow_experiment_name)
            mlflow.start_run(run_name=self.strategy.args.mlflow_run_name if self.strategy.args.mlflow_run_name else self.strategy.args.save_path)
            # Log configuration parameters.
            mlflow.log_params(self.strategy.args.__dict__)
        ###############################################################################################
    ...
    def fit(self, args, consumed_samples=0, num_update_steps_per_epoch=None):
        ...
        ###############################################################################################
        if self._mlflow is not None and self.strategy.is_rank_0():
    ...
    def save_logs_and_checkpoints(self, args, global_step, step_bar, logs_dict={}, client_states={}):
        if global_step % args.logging_steps == 0:
            ...
            # MLFLOW
            if self._mlflow is not None and self.strategy.is_rank_0():
                mlflow_logs = {f"train/{k}": v for k, v in logs_dict.items()}
                mlflow_logs["train/global_step"] = global_step
                # Log all metrics at once; each metric will be associated with the same step.
                self._mlflow.log_metrics(mlflow_logs, step=global_step)
            ###############################################################################################
        ...
        with torch.no_grad():
            ...
            if self.strategy.is_rank_0():
                ...
                if self._mlflow is not None:
                    mlflow_eval_logs = {f"eval/{k}": v for k, v in logs.items()}
                    mlflow_eval_logs["eval/global_step"] = steps
                    self._mlflow.log_metrics(mlflow_eval_logs, step=steps)
                ###############################################################################################
