...
def main():
    ...
    save_dir = os.path.join(args.save_dir, data_name, str(args.m))
    OUT_NAME = f'{model_name}_m_{args.m}.json'
    ...
    ### Load model
    model = LLM(model=args.model_path, tensor_parallel_size=args.world_size, enable_prefix_caching=False, enforce_eager=True)
    ...
    for i in range(args.m):
        sampling_params = SamplingParams(max_tokens=args.max_tokens, temperature=args.temperature, stop=tokenizer.eos_token, seed=i)
        print("Sampling Params: ", sampling_params)
        ### Generation
        print(f"Generate {i}-th response...")
    ...
    invalid_output = 0
    total_len = 0
    for sample_idx in range(len(all_results[0])):
        strict_num_A, strict_num_B = 0, 0
        for res_idx, results in enumerate(all_results):
            eval_data[sample_idx][f"response_{res_idx}"] = results[sample_idx]
            eval_data[sample_idx][f"token_length_{res_idx}"] = len(tokenizer(results[sample_idx], add_special_tokens=False).input_ids)
            total_len += eval_data[sample_idx][f"token_length_{res_idx}"]
            if parse_func_strict(results[sample_idx]) == 'A':
                strict_num_A += 1
            elif parse_func_strict(results[sample_idx]) == 'B':
                strict_num_B += 1
        if strict_num_A == 0 and strict_num_B == 0:
            eval_data[sample_idx]["strict_predict"] = None
            invalid_output += 1
        else:
            eval_data[sample_idx]["strict_predict"] = 'A' if strict_num_A > strict_num_B else ('B' if strict_num_B > strict_num_A else 'C')

        strict_domain_scores[eval_data[sample_idx]["domain"]].append(1.0 if eval_data[sample_idx]["strict_predict"] == eval_data[sample_idx]["label"] else (0.5 if eval_data[sample_idx]["strict_predict"] == 'C' else 0.0))
        flexible_domain_scores[eval_data[sample_idx]["domain"]].append(1.0 if eval_data[sample_idx]["strict_predict"] == eval_data[sample_idx]["label"] else (0.5 if (eval_data[sample_idx]["strict_predict"] == None) or (eval_data[sample_idx]["strict_predict"] == 'C') else 0.0))

        if "RM-Bench" in args.dataset:
            strict_difficulty_scores[eval_data[sample_idx]["difficulty"]].append(1.0 if eval_data[sample_idx]["strict_predict"] == eval_data[sample_idx]["label"] else (0.5 if eval_data[sample_idx]["strict_predict"] == 'C' else 0.0))
            flexible_difficulty_scores[eval_data[sample_idx]["difficulty"]].append(1.0 if eval_data[sample_idx]["strict_predict"] == eval_data[sample_idx]["label"] else (0.5 if (eval_data[sample_idx]["strict_predict"] == None) or (eval_data[sample_idx]["strict_predict"] == 'C') else 0.0))
    
    invalid_output_ratio = invalid_output / len(eval_data)
    avg_response_length = total_len / (args.m * len(eval_data))
    ...
    if "HelpSteer2" in args.dataset or "HelpSteer3" in args.dataset:
