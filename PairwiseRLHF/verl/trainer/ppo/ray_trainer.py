...
def compute_advantage_from_difference(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1):
    ...
    if adv_estimator == AdvantageEstimator.GRPO:
        advantages, returns = core_algos.compute_grpo_difference_advantage(
            reward_diff=data.batch['reward_diff'],
        ...
        # 1) Does x contain any NaNs?
        nan_mask = torch.isnan(data.batch['advantages'])
        if nan_mask.any():
            print("There is at least one NaN in x")
        else:
            print("There is no NaN in x")
        # 1) Does x contain any NaNs?
        inf_mask = torch.isinf(data.batch['advantages'])
        if inf_mask.any():
            print("There is at least one Inf in x")
        else:
            print("There is no Inf in x")
    else:
    ...
    def _validate(self):
        ...
        for data_source, var2metric2val in data_src2var2metric2val.items():
            ...
            for var_name, metric2val in var2metric2val.items():
                ...
                n_max = max([int(name.split("_at_")[-1].split("/")[0]) for name in metric2val.keys()])
                for metric_name, metric_val in metric2val.items():
                    if (var_name == core_var) and any(
                            # metric_name.startswith(pfx) for pfx in ["mean", "maj", "best"]) and (f"@{n_max}"
                            #                                                                      in metric_name):
                            metric_name.startswith(pfx) for pfx in ["mean", "maj", "best"]) and (f"_at_{n_max}"
                                                                                                 in metric_name):
                        metric_sec = "val-core"
                    else:
                        metric_sec = "val-aux"
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        try:
                            ...
                            reward_diff_tensor = reward_result['reward_diff_tensor']
                            reward_extra_infos_dict = reward_result['reward_extra_info']
                        except Exception as e:
                            print(f'Error in reward_fn: {e}')
                            # reward_tensor = self.reward_fn(batch)
                            reward_diff_tensor = self.reward_fn(batch)
                            reward_extra_infos_dict = {}

                        # batch.batch['token_level_scores'] = reward_tensor
                        batch.batch['score_diff'] = reward_diff_tensor

                        ### reward_tensor.shape: (n * rollout_size, max_response_length)
                        ### reward_diff_tensor.shape: (n * rollout_size, n * rollout_size)
                        # nonzero_counts = (reward_tensor != 0).sum(dim=1)
                        ...
                        if self.config.algorithm.use_kl_in_reward:
                            ...
                            batch.batch['reward_diff'] = batch.batch['score_diff']
                        ...
                        batch = compute_advantage_from_difference(batch,
                                                  adv_estimator=self.config.algorithm.adv_estimator,
                                                  gamma=self.config.algorithm.gamma,
                                                  lam=self.config.algorithm.lam,
                                                  num_repeat=self.config.actor_rollout_ref.rollout.n)
