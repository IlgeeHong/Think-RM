...
def compute_grpo_difference_advantage(reward_diff: torch.Tensor,
                                      response_mask: torch.Tensor,
                                      num_repeat: int,
                                      epsilon: float = 1e-6):
    """
    Compute advantage A_i from a full (bs * bs) reward_diff matrix.

    Args:
        reward_diff: FloatTensor of shape (bs, bs), where reward_diff[i, j] = r_i - r_j if i,j in same group, else NaN.
        response_mask: Tensor of shape (bs, response_length)
        num_repeat: int, group size G (number of responses per prompt).
    ...
    Returns:
        ...
        Returns: `(torch.Tensor)`
            shape: (bs, response_length)
    """
    device = reward_diff.device
    N = reward_diff.size(0)
    G = num_repeat
    if N % G != 0:
        raise ValueError(f"Total elements N={N} is not divisible by group size G={G}")
    # B = N // G
    ### compute numerator
    numer = torch.nansum(reward_diff, dim=1) / G   # shape (N,)

    visited   = torch.zeros(N, dtype=torch.bool, device=device)
    advantage = torch.zeros(N, device=device)

    for i in range(N):
        if visited[i]:
            continue

        # 2) discover group from NaN mask
        #    members = all j where reward_diff[i,j] is not NaN
        members = torch.nonzero(~torch.isnan(reward_diff[i]), as_tuple=True)[0]
        # include i since diagonal terms are always NaN
        if i not in members:
            print("debug: diagonal terms were NaN")
            members = torch.cat([members, torch.tensor([i], device=device)])

        if members.numel() != G:
            raise ValueError(f"group at i={i} has size {members.numel()}, expected {G}")

        visited[members] = True

        # 3) extract block (now contains only real numbers)
        block = reward_diff[members][:, members]  # (G,G)
        # block.fill_diagonal_(0.0) # 
        S2 = (block ** 2).sum()
        denom = torch.sqrt(S2 / (2 * G * (G-1)) + epsilon)

        advantage[members] = numer[members] / denom

    # print("advantage", advantage)
    # 4) mask & reshape
    adv = advantage.unsqueeze(-1) * response_mask
    return adv, adv
