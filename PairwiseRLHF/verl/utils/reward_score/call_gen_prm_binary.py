...
from verl.utils.reward_score.prompt_templates import NaiveReasoningBinaryPromptTemplate
import requests
import re
import math
...
def extract_binary_strict(text):
    ...
    return -1/len(text) if str(matches[-1]) == 'A' else (1/len(text) if str(matches[-1]) == 'B' else None)
...
def _compute_chunk(server_url, prompt_template, prompts, response_pairs, model_name, temperature=0.6, max_tokens=2048, n=1):
    ...
    for prompt, (r1, r2) in zip(prompts, response_pairs):
        text = chat_vllm_server(server_url, prompt_template, prompt, r1, r2, model_name, temperature, max_tokens, n)
        score = extract_binary_strict(text)
        sub_scores.append(score if score is not None else 0)
    return sub_scores
...
def compute_pref_batched(prompts, data_sources, response_pairs, extra_infos, model_name, vllm_server_ip="localhost", start_port=8000, n_port=8, temperature=0.6, max_tokens=2048, n=1):
    ...
    M = len(prompts)
    n_servers = n_port
    server_urls = [f"http://{vllm_server_ip}:{start_port + i}/v1/chat/completions" for i in range(n_port)]
    chunk_size = math.ceil(M / n_servers)
    template = NaiveReasoningBinaryPromptTemplate()
